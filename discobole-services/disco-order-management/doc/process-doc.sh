...
function process_module() {
  ...
  for resources_path in $resources_paths; do
    ...
    spec_files=$(grep -l -e 'openapi: 3' -e 'openapi: "3' -e 'swagger: 2' -e 'swagger: "2' "$resources_path"/*.yaml 2>/dev/null) # may have several spec files in src/main/resources (sample: user-role API)
...
# Iterates over all projects from given group with given visibility and tries to process it as a module project
# $1: GitLab group path
...
function process_modules_from_group() {
  ...
  visibility="$2"
  exclude_projects="$3"
  group_name=$(echo "$group_path" | sed -e 's/disco-order-management\///g')
...
function build_architecture_decisions_toc() {
  ...
  git clone -q --depth 1 "https://gitlab-ci-token:${CI_JOB_TOKEN}@${CI_SERVER_HOST}/disco/disco-oda-components/disco-order-management/order-architecture.git" "$adr_src_dir"
  status="$?"
  if [ "$status" != 0 ]; then 
...
function build_detailed_architecture_toc() {
  ...
  if [[ -f "$tmp_dir/toc/readmes" ]]; then
    ...
    # sort by API project names then by OpenShift project names
    MD_INDENT="    "
    {
      # apis and libs
      grep -e "- order" "$tmp_dir/toc/readmes" | sed "s/^-/${MD_INDENT}- API >/"
    } >> "$toc_file"
  fi
}
...
function build_api_conformance_reports_toc() {
  log_info "--- Generating API conformance reports TOC..."
  toc_file="$tmp_dir/api_conformance_report_toc.md"
  touch "$toc_file"
  # sort by API project names
  MD_INDENT="    "
...
function build_api_compliance_reports_toc() {
  ...
  toc_file="$tmp_dir/api_compliance_report_toc.md"
  touch "$toc_file"
  # sort by API project names
  MD_INDENT="    "
...
log_info "- Open source        (--open-source-enabled)    : \\e[33;1m${OPEN_SOURCE_ENABLED}\\e[0m"
...
if [[ -f "mkdocs.yml" ]]; then
  ...
  toc=$(< "$tmp_dir/architecture_decisions_toc.md" sed 's/^/  /' | awk '{gsub(/&/, "\\\\&"); printf "%s\\n", $0}')
  awk -v toc="${toc}" '/INSERT_ARCHITECTURE_DECISIONS_TOC_HERE/ {print toc; next} 1' mkdocs.yml > "${tmp_dir}/mkdocs-with-toc-tmp1.yml"
  ...
  toc=$(< "$tmp_dir/api_conformance_report_toc.md" sed 's/^/  /')
  awk -v toc="${toc//&/\\\\&}" '{sub(/.*INSERT_API_CONFORMANCE_REPORT_TOC_HERE.*/,toc)}1' "${tmp_dir}/mkdocs-with-toc-tmp3.yml" > "${tmp_dir}/mkdocs-with-toc-tmp4.yml"

  log_info "Replacing API compliance report toc in mkdocs.yml..."
  toc=$(< "$tmp_dir/api_compliance_report_toc.md" sed 's/^/  /')
  awk -v toc="${toc//&/\\\\&}" '{sub(/.*INSERT_API_COMPLIANCE_REPORT_TOC_HERE.*/,toc)}1' "${tmp_dir}/mkdocs-with-toc-tmp4.yml" > "${tmp_dir}/mkdocs-with-toc-tmp5.yml"
...
## ----------------------------------------------------------------------------
##  Commit generated pages to skip the page building for OW2
## ----------------------------------------------------------------------------
