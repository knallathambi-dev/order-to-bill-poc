...
DISCOBOLE Order Capture microservice implements the TMFC002 Product Order Capture and Validation and developed with Spring
libraries (`Spring Boot`, `Spring Web`, `Spring Data`).

The `Order Capture` service implements the TMF701 Process FLow Management to:

- **capture orders**: collects customer-specific data, including product selection, configurations, and preferences.
- **validate orders**: ensures the order aligns with business rules, checks for service eligibility, and verifies data
  accuracy.
...
Your Maven repository may require authentication credentials to publish artifacts in unstable and stable maven
repositories. To handle the authentication, please read
the [to-be-continuous maven template documentation](https://to-be-continuous.gitlab.io/doc/ref/maven/#maven-repository-authentication)
...
```bash
  git clone git@gitlab.ow2.org:discobole/disco-oda-components/disco-order-management/order-capture.git
```
...
```bash
  curl -s https://gitlab.ow2.org/api/v4/projects/discobole%2Fdisco-oda-components%2Fdisco-order-management%2Forder-capture | jq .id
```
...
The default configuration is set up in `src/main/resources/config/application.yml` file.
...
**Note:** before running the application, ensure that both MongoDB and Kafka are started and running on your local machine or server.

### Usage

Access to the [OpenAPI specification](http://localhost:8086/swagger-ui/index.html#/) of the API.
...
Please consult its [documentation](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/helm/chart/README.md).
...
included [gitlab-ci.yml](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/.gitlab-ci.yml) file includes some templates provided
...
by each template is documented via comments in the [gitlab-ci.yml](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/.gitlab-ci.yml) file.
...
Please read [CONTRIBUTING.md](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/CONTRIBUTING.md) for details on our process for submitting merge requests to us.

## Versioning

We use [SemVer](http://semver.org/) for versioning. For the versions available, see
the [tags on this repository](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/tags).

## Authors

See the list of [contributors](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/CONTRIBUTORS.md) who participated in this project.

## License

This project is licensed - see the [LICENSE](https://gitlab.ow2.org/discobole/disco-oda-components/disco-order-management/order-capture/-/blob/main/LICENSE.txt) file for details.
