...
${name}     Samsung Galaxy Buds2 ear buds
*** Test Cases ***

Aquisition Samsung Buds InStore
    ...
    [Tags]                     Test_Suite_Samsung_Buds_Aquisition
     Given This token must be created
                                 ${Product_Offering_Id_Bud}      for Accessories the customer get product id from catalog for ${name}
                                 # Step 1 : Create Process Flow
                                 ${response}    the customer create a product Configuration for ${Product_Offering_Id_Bud}
         ...
         ${request_body}         And the user enters the value "${Product_Offering_Id_Bud}" in the "id" field on the path "[0]['value']" in the "${request_body}"
...
Aquisition Samsung Buds Home delivery
    [Documentation]         This test case consists of testing the acquisition of a prepaid offer for an identified customer by demonstrating the necessary steps.
    [Tags]                     Test_Suite_Samsung_Buds_Aquisition
      Given This token must be created
                                  ${Product_Offering_Id_Bud}      for Accessories the customer get product id from catalog for ${name}
