...
public interface ProductOfferingService {
    ...
    ResponseResult isProductOfferingExist(String productOfferingId, ProductOfferingResultValidator validator);

    @Retryable(retryFor = Exception.class, maxAttemptsExpression = "${retry.maxAttempts}", backoff = @Backoff(delayExpression = "${retry.maxDelay}"))
    ProductOffering getProductOfferingById(String productOfferingId);
}