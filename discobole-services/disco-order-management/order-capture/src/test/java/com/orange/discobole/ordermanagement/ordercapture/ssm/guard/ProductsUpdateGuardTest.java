...
class ProductsUpdateGuardTest {
    @InjectMocks
    private ProductsUpdateGuard productsUpdateGuard;

    @DisplayName("given re-executed products update guard " +
            "when check if products were updated " +
            "then return true")
    @Test
    void testProductsUpdateGuardReExecution() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);
        StateMachineUtil.setGuardContext(context, Boolean.TRUE, GuardNameConstants.PRODUCTS_UPDATE_GUARD);
        context.getExtendedState().getVariables().put(StateMachineUtil.ON_METHOD, Boolean.TRUE);

        //when
        Mono<Boolean> result = productsUpdateGuard.apply(context);
    ...
    @DisplayName("given the products not updated " +
            "when check if products were updated " +
            "then the result of the guard in context is set to false " +
            "and return false")
    @Test
    void testProductsNotUpdated() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);

        //when
        Mono<Boolean> result = productsUpdateGuard.apply(context);
        Mono<Boolean> guardResultContext = StateMachineUtil.getGuardResult(context, GuardNameConstants.PRODUCTS_UPDATE_GUARD);
    ...
    @DisplayName("given the products were updated " +
            "when check if products were updated " +
    ...
    private DefaultStateContext<String, String> mockStatContext(boolean isProductOrderUpdated) {
        ...
        extendedState.getVariables().put(OrderCaptureConstants.ARE_PRODUCTS_UPDATED, isProductOrderUpdated);
