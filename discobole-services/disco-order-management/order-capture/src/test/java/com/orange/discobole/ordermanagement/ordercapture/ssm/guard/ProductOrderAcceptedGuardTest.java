...
class ProductOrderAcceptedGuardTest {
    @InjectMocks
    private ProductOrderAcceptedGuard productOrderAcceptedGuard;

    @DisplayName("given re-executed product order accepted guard " +
            "when check if product order state is accepted " +
            "then return true")
    @Test
    void testProductOrderUpdateGuardReExecution() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);
        StateMachineUtil.setGuardContext(context, Boolean.TRUE, GuardNameConstants.PRODUCT_ORDER_ACCEPTED_GUARD);
        context.getExtendedState().getVariables().put(StateMachineUtil.ON_METHOD, Boolean.TRUE);

        //when
        Mono<Boolean> result = productOrderAcceptedGuard.apply(context);
    ...
    void testProductOrderNotUpdated() {
        ...
        //when
        Mono<Boolean> result = productOrderAcceptedGuard.apply(context);
        Mono<Boolean> guardResultContext = StateMachineUtil.getGuardResult(context, GuardNameConstants.PRODUCT_ORDER_ACCEPTED_GUARD);
    ...
    private DefaultStateContext<String, String> mockStatContext(boolean isProductOrderAccepted) {
        ExtendedState extendedState = new DefaultExtendedState();
        extendedState.getVariables().put(OrderCaptureConstants.IS_PRODUCT_ORDER_ACCEPTED, isProductOrderAccepted);
