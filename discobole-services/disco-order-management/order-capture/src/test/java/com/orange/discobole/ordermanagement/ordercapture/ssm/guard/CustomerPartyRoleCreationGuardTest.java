...
class CustomerPartyRoleCreationGuardTest {
    @InjectMocks
    private CustomerPartyRoleCreationGuard customerPartyRoleCreationGuard;

    @DisplayName("given re-executed customer party role created guard " +
            "when check if party role customer is created " +
            "then return true")
    @Test
    void testCustomerPartyRoleCreationGuardReExecution() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);
        StateMachineUtil.setGuardContext(context, Boolean.TRUE, GuardNameConstants.CUSTOMER_PARTY_ROLE_CREATION_GUARD);
        context.getExtendedState().getVariables().put(StateMachineUtil.ON_METHOD, Boolean.TRUE);

        //when
        Mono<Boolean> result = customerPartyRoleCreationGuard.apply(context);
    ...
    @DisplayName("given customer party role not created " +
            "when check if related role customer is created " +
            "then the result of the guard in context is set to false " +
            "and return false")
    @Test
    void testCustomerPartyRoleNotCreated() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);

        //when
        Mono<Boolean> result = customerPartyRoleCreationGuard.apply(context);
        Mono<Boolean> guardResultContext = StateMachineUtil.getGuardResult(context, GuardNameConstants.CUSTOMER_PARTY_ROLE_CREATION_GUARD);
    ...
    private DefaultStateContext<String, String> mockStatContext(boolean isCustomerPartyRoleCreated) {
        ...
        extendedState.getVariables().put(OrderCaptureConstants.IS_CUSTOMER_PARTY_ROLE_CREATED, isCustomerPartyRoleCreated);
