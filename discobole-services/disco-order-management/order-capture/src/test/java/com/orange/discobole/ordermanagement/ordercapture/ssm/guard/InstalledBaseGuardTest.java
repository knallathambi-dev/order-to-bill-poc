...
class InstalledBaseGuardTest {
    @InjectMocks
    private InstalledBaseGuard installedBaseGuard;

    @DisplayName("Given re-executed products created guard " +
            "when check if products are created " +
            "then return true")
    @Test
    void testProductOrderUpdateGuardReExecution() {
        //given
        StateContext<String, String> context = mockStatContext(Boolean.FALSE);
        StateMachineUtil.setGuardContext(context, Boolean.TRUE, GuardNameConstants.INSTALLED_BASE_GUARD);
        context.getExtendedState().getVariables().put(StateMachineUtil.ON_METHOD, Boolean.TRUE);

        //when
        Mono<Boolean> result = installedBaseGuard.apply(context);
    ...
    @DisplayName("Given the products   not created " +
            "when check if products are created " +
    ...
    void testProductOrderNotUpdated() {
        ...
        //when
        Mono<Boolean> result = installedBaseGuard.apply(context);
        Mono<Boolean> guardResultContext = StateMachineUtil.getGuardResult(context, GuardNameConstants.INSTALLED_BASE_GUARD);
    ...
    @DisplayName("Given the products was created " +
            "when check if products are created " +
    ...
    private DefaultStateContext<String, String> mockStatContext(boolean areProductsCreated) {
