...
function process_module() {
  ...
  log_info "Project \\e[32m${project_path}\\e[0m declares module \\e[32m${project_name}\\e[0m of kind \\e[33;1m${group_name}\\e[0m on \\e[33;1m${project_branch}\\e[0m branch"
  ...
  http_status=$(curl "$CURL_SILENT" -R -H "$AUTH_HEADER" --output "$readme_filename" -w "%{http_code}" "$API_URL/projects/$project_id/repository/files/README.md/raw?ref=$project_branch")
  ...
  if [ "$http_status" == 200 ]; then
    ...
    # README.md might contain relative paths => use absolute paths for OW2 GitLab instance
    ...
    # README.md might contain planuml diagrams => replace them by mkdocs puml
  ...
  for resources_path in $resources_paths; do
    ...
    spec_files=$(grep -l -e 'openapi: "3' -e "openapi: '3" -e 'openapi: 3' -e 'swagger: "2' -e "swagger: '2" -e 'swagger: 2' "$resources_path"/*.yaml 2>/dev/null) # may have several spec files in src/main/resources (sample: user-role API)
...
# Iterates over all projects from given group with given visibility and tries to process it as a disco-order-management module project
# $1: GitLab group path
...
function process_modules_from_group() {
  ...
  visibility="$2"
  exclude_projects="$3"
  group_name=$(echo "$group_path" | sed -e 's/disco-catalog\///g')
  log_info "--- Looking for projects with visibility \\e[33;1m${visibility}\\e[0m from group \\e[32m${group_path}\\e[0m..."

  projects_b64=$(curl "$CURL_SILENT" -S -f -H "$AUTH_HEADER" "$API_URL/groups/$group_id/projects?archived=false&visibility=${visibility}&per_page=100" | jq -r '.[] | @base64')

  for project_b64 in $projects_b64
  do
...
function build_architecture_decisions_toc() {
  log_info "--- Get ADR markdown files from architecture repository ..."
  adr_src_dir="$tmp_dir/architecture"
  git clone --depth 1 "https://gitlab-ci-token:${CI_JOB_TOKEN}@${CI_SERVER_HOST}/disco/disco-oda-components/disco-catalog/catalog-architecture.git" "$adr_src_dir"

  # get adr markdown files
  adr_dst_dir=$DOC_OUT/architecture/decisions
  mkdir -p "$adr_dst_dir"
  rm "$adr_dst_dir"/*.md 2>/dev/null
  ...
  for adr_file in "$adr_dst_dir"/*.md; do
    if [ -f "$adr_file" ]; then
      log_info " ... processing \\e[33;1m$adr_file\\e[0m"
      sed -i -e "s/([^0-9]*/(..\//g" -e "s/\.md)/)/g" "$adr_file"
    else
      log_info " ... no ADR files found in \\e[33;1m$adr_dst_dir\\e[0m"
    fi
  done
  ...
  do
    adr_label=ADR-$(basename "$adr_file" .md)
...
# process all disco-order-management module toc
...
then
  ...
  awk -v toc="${toc//&/\\\\&}" '{sub(/.*INSERT_API_CONFORMANCE_REPORTS_TOC_HERE.*/,toc)}1' "${tmp_dir}/mkdocs-with-toc-tmp4.yml" > "${tmp_dir}/mkdocs-with-toc-tmp5.yml"
...
## add user name and email as required for the commit and push of the changes
log_info "Configure user name and email for git commit and puhh..."
if [[ -z $(git config --get user.name) ]]; then
  log_info "--- configure user name for git commit and push..."
  log_info "--- configure email for git commit and push..."
  username=$(echo "$CI_COMMIT_AUTHOR" | awk -F "\<" '{print $1}')
  email=$(echo "$CI_COMMIT_AUTHOR" | awk -F "\<" '{print $2}' | sed 's/>//')
  git config --global user.email "$username"
  git config --global user.name "$email"
else
  log_info "Git user name and email already configured: $(git config --get user.name) <$(git config --get user.email)>"
fi
