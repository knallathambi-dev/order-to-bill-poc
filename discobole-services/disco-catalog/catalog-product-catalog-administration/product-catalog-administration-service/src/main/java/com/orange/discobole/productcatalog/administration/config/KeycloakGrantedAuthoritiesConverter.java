...
import com.orange.discobole.productcatalog.administration.handler.RestTemplateResponseErrorHandler;
import com.orange.discobole.productcatalog.administration.interceptor.RestTemplateInterceptor;
import com.orange.discobole.productcatalog.administration.dto.generated.common.UserRole;
import com.orange.discobole.productcatalog.administration.dto.generated.common.Entitlement;
...
//    @Autowired
    ...
    public Collection<GrantedAuthority> convert(Jwt source) {
        ...
        return entitlements.stream().map(rn -> new SimpleGrantedAuthority(rn)).collect(Collectors.toList());
    }

    private List<String> getEntitlements(List<String> roles,String token ) {
        //List<UserRole> userRoles = fetchUserRoles(roles);
    ...
    public List<UserRole> fetchUserRoles2(List<String> clientRoles) {
        String marketUrl = UriComponentsBuilder.fromUriString(userRoleRetrievalUrl)
                .queryParam("involvementRole", clientRoles).toUriString();
        List<UserRole> roles = getRestTemplate()
                .exchange(marketUrl, HttpMethod.GET, null, new ParameterizedTypeReference<List<UserRole>>() {
                }).getBody();
        return roles;
    }
    ...
    public List<UserRole> fetchUserRoles(String token) {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                LOGGER.info("User Role Fetched Count : {}", response.getBody().size());
                LOGGER.info("content type : {}", response.getHeaders() != null ? response.getHeaders().getContentType() : "null");
                return response.getBody();
            }
            LOGGER.info("User Role Fetched Failed : {}", response.getStatusCode());
        } catch (Exception e) {
            LOGGER.error("Exception while fetching user roles", e);
        }
