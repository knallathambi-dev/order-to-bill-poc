...
public class ResourceServerConfiguration {
    ...
    private static final String[] ENDPOINTS_PATHS = {
            "/unit", "/unit/**", "/marketSegment", "/marketSegment/**", "/currency", "/currency/**", "/frequencyTypes", "/frequencyTypes/**", "/productInventoryCheck", "/productInventoryCheck/**", "/channel", "/channel/**"
    };

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(requests -> requests.requestMatchers(CLASSPATH_RESOURCE_LOCATIONS).permitAll()
                .requestMatchers(ENDPOINTS_PATHS)
