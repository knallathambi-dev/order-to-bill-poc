// SPDX-FileCopyrightText: 2025 Orange SA
// SPDX-License-Identifier: MIT
//
// This software is distributed under the MIT License,
// the text of which is available at https://opensource.org/license/mit
// or see the "LICENSE.txt" file for more details.
//
// Authors: See CONTRIBUTORS.txt

package com.orange.discobole.productcatalog.productspecification.event.productspec;


import com.orange.discobole.processflow.event.Event;
import com.orange.discobole.productcatalog.productspecification.constant.OdacaConstants;



/**
 * This interface gives the events related to product specification a common
 * name
 *
 * @author Piyush Goel
 * @since 1.0
 */
public interface ProductSpecEvent extends Event {

	/**
	 * This method is used to get the aggregate name.
	 *
	 * @return returns the aggregate name
	 */
	@Override
	default String aggregateName() {
		return OdacaConstants.PRODUCT_SPEC;
	}
}
