...
@Component
public class CategoryProductOfferingListener {


    private static final Logger LOGGER = LogManager.getLogger(CategoryProductOfferingListener.class);
    ...
    @PostConstruct
    private void init(){
    	register(CategoryProductOfferingAssociationEvent.class, eventHandler::handle);

    }


    @Bean
    public Consumer<Message<EventData>> categoryProductOffering() {
        return message -> {
            LOGGER.info("Received message on topic category.categoryProductOffering containing event {}", message.getPayload().getEvent());
