...
class ModifySelectCategoryTypeActionTest extends CategoryApplicationTests {
	
	@InjectMocks
	private ModifySelectCategoryTypeAction modifySelectCategoryTypeAction;
	@Mock
	...
	public void init() throws JsonMappingException, JsonProcessingException, JSONException, Exception {
		ReflectionTestUtils.setField(modifySelectCategoryTypeAction, "objectMapper", objectMapper);
		modifySelectCategoryTypeAction.init();
	}
	...
	public void performTest() {
		...
		if(selectEntityType.getCategoryType() == CategoryEntityType.PRODUCTOFFERINGCATEGORY) {
			Map<String, Object> performVariables = modifySelectCategoryTypeAction.perform(stateMachineTransition,
					taskFlowUpdate);
			assertThat(performVariables.get(CategoryConstants.CATEGORY_TYPE)).isEqualTo("ProductOfferingCategory");
			assertEquals(List.of(new StringCharacteristic().value(selectEntityType.getCategoryType().getValue()).name(CategoryConstants.CATEGORY_TYPE)
	...
	public void performCharateristicGetTest() {
		StateMachineTransition stateMachineTransition = new StateMachineTransition();
		List<CharacteristicSpecification> charact = modifySelectCategoryTypeAction
