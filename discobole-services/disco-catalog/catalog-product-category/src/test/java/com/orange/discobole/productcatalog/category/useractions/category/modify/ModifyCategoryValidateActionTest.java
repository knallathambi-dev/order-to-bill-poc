...
public class ModifyCategoryValidateActionTest extends CategoryApplicationTests {
@InjectMocks
private modifyCategoryValidateAction categoryValidateAction;
@Mock
...
public void init() throws JsonMappingException, JsonProcessingException, JSONException, Exception {
	...
	stateMachineTransition.setTaskDefinitionKey("defineValidityPeriod");
	stateMachineTransition.setVariablesFromUserActions(variablesFromUserActions);
	ReflectionTestUtils.setField(categoryValidateAction, "objectMapper", objectMapper);
	categoryValidateAction.init();
}
@Test
void performTest() {
	TaskFlowUpdate taskFlowUpdate = new TaskFlowUpdate();
	List<Characteristic> characteristicList = new ArrayList<>();
	ValidateEntityOperation validateEnitity = new ValidateEntityOperation();
	validateEnitity.setValidated(true);
	Characteristic characteristic = new ObjectCharacteristic().value(validateEnitity).valueType("Object")
			.name(CategoryConstants.VALIDATE);
	characteristicList.add(characteristic);
	taskFlowUpdate.setCharacteristic(characteristicList);
	categoryValidateAction.perform(stateMachineTransition, taskFlowUpdate);
	Mockito.verify(modifyCategoryService, Mockito.times(1)).validateModifyCategory(CATEGORY_ID);

}

@Test
void requiredCharacteristicsTest() {
	categoryValidateAction.requiredCharacteristics(stateMachineTransition, null);
	List<CharacteristicSpecification> list= categoryValidateAction.requiredCharacteristics(stateMachineTransition, null); 
	 assertNotNull(list);
