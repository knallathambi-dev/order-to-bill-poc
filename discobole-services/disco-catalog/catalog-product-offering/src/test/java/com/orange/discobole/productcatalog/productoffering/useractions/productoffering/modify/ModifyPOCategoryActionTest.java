...
import com.orange.discobole.productcatalog.productoffering.useractions.productoffering.modify.ModifyPOCategoryAction;

class ModifyPOCategoryActionTest extends ProductOfferingApplicationTests {

	@InjectMocks
	private ModifyPOCategoryAction modifyProductOfferingCategoryAction;
	...
	@BeforeEach
	void init() {
		ReflectionTestUtils.setField(modifyProductOfferingCategoryAction, "objectMapper", objectMapper);
		productOffering.setId(PRODUCTOFFID);
		productOffering.setLifecycleStatus(ProductOfferingLifecycle.ACTIVE);
		modifyProductOfferingCategoryAction.init();
	}
	...
	void performProductOfferingCategoryWhenUserEntersValues() {
		...
		stateMachineTransition.setTaskDefinitionKey("defineProductOfferingCategory");
		...
		DefineProductOfferingCategory category = new DefineProductOfferingCategory();
		category.setProductCategoryId("category1");
		Characteristic charac = new ObjectCharacteristic().value(List.of(category)).name(ProductOffConstants.CATEGORY);
		characteristicList.add(charac);
		taskFlowUpdate.setCharacteristic(characteristicList);
		Map<String, Object> performVariables = modifyProductOfferingCategoryAction.perform(stateMachineTransition,
				taskFlowUpdate);
		assertThat(performVariables).isEmpty();
		verify(productOfferingService).modifyProductOfferingCategory(PRODUCTOFFID, List.of("category1"));
	}
	
	 @Test
	    void requiredCharacteristicsTest() {
	        characteristicList = modifyProductOfferingCategoryAction
