...
import com.orange.discobole.productcatalog.productoffering.useractions.productoffering.DefineProductOfferingCategoryAction;

class DefineProductOfferingCategoryActionTest extends ProductOfferingApplicationTests {

	@InjectMocks
	private DefineProductOfferingCategoryAction defineProductOfferingCategoryAction;
	...
	@BeforeEach
	void init() {
		ReflectionTestUtils.setField(defineProductOfferingCategoryAction, "objectMapper", objectMapper);
		defineProductOfferingCategoryAction.init();
	}
	...
	void performProductOfferingCategoryWhenUserEntersValues() {
		...
		Map<String, Object> performVariables = defineProductOfferingCategoryAction.perform(stateMachineTransition,
				taskFlowUpdate);
		assertThat(performVariables).isEmpty();
		verify(productOfferingService).defineProductOfferingCategory(PRODUCTOFFID, List.of("category1"));
	}
