...
class CharacteristicUtilTest extends ProductOfferingApplicationTests {
	...
	void init() {
		...
		try {
			String file = FileUtil.read("/schemas/" + PickUsageSpecification.class.getSimpleName() + ".json");
	...
	void getCharacteristicWhenNameNullTest() throws Exception {
		List<Characteristic> list=taskFlowUpdate.getCharacteristic();
		assertThrows(IllegalArgumentException.class, () -> CharacteristicUtil.getCharacteristic(list, null, characteristicList));
	}
