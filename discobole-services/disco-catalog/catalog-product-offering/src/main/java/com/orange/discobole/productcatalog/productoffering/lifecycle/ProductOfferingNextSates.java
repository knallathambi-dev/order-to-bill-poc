...
public class ProductOfferingNextSates {
	...
	public Set<String> getNextPossibleStates(String currentState, List<ProductOffering> bundleProductOfferings) {
		...
		case "active":
			...
			break;
		case "launched":
			if (bundleProductOfferings == null || bundleProductOfferings.isEmpty()) {
				// No bundles: atomic PO — allow both transitions
				possibleStates.add(STATE_UNAVAILABLE);
				possibleStates.add(RETIRED);
			} else {
	...
	private boolean validateStatus(List<ProductOffering> bundleProductOfferings) {
		for (ProductOffering bundleProductOffering : bundleProductOfferings) {
			ProductOfferingLifecycle status = bundleProductOffering.getLifecycleStatus();
