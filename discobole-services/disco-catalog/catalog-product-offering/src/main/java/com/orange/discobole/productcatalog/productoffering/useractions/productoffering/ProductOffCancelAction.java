 ...
 *
 */
@Component("ProductOfferingCreation.cancel")
public class ProductOffCancelAction implements UserAction {

	private static final Logger LOGGER = LogManager.getLogger(ProductOffCancelAction.class);
	...
	public Map<String, Object> perform(StateMachineTransition stateMachine, TaskFlowUpdate taskFlowUpdate)
		...
		LOGGER.info("ProductOffCancelAction perform method");
		...
		if (null != characteristic) {
			...
			if (cancelData.isIsCancelled().equals(Boolean.TRUE)) {
				...
				if (productOffId != null) {
					productOffService.cancelProductOff(productOffId);
				}
			}
		}
