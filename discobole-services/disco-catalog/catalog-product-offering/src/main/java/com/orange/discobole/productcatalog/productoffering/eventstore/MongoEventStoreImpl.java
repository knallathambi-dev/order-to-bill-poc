...
public class MongoEventStoreImpl implements MongoEventStore {
	...
	@PostConstruct
	public void init() {
		LOGGER.info("mongoclient: {}", appCtx.getBean("mongo", MongoClient.class));

		MongoClient client = (MongoClient) appCtx.getBean("mongo");
		client.listDatabaseNames()
				.forEach(dbName -> LOGGER.info("Find MongoDb Database: {}",dbName));

		LOGGER.info("eventStorageEngine: {}", appCtx.getBean("eventStorageEngine", MongoEventStorageEngine.class));
