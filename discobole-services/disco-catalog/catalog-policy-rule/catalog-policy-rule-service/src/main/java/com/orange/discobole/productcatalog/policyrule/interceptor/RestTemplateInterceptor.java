...
package com.orange.discobole.productcatalog.policyrule.interceptor;
...
public class RestTemplateInterceptor implements ClientHttpRequestInterceptor {
	...
	private void logRequestDetails(final HttpRequest request, final byte[] body) {
		CompletableFuture.runAsync(()->{
			...
			LOGGER.info("Request info - {}", logBuilder);
		});
	}
	 ...
	 */
	private void logResponseDetails(final ClientHttpResponse response) {
		CompletableFuture.runAsync(()->{
			final StringBuilder logBuilder = new StringBuilder();
			try {
			...
			LOGGER.info("Response info - {}", logBuilder);
		});
	}
}
