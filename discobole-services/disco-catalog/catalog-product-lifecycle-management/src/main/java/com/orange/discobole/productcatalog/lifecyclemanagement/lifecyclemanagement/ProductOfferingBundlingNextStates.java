...
public class ProductOfferingBundlingNextStates {
	...
	private boolean validateStatus(List<ProductOffering> bundleProductOfferings) {
		...
		for (ProductOffering bundleProductOffering : bundleProductOfferings) {
			if (null!=bundleProductOffering && !bundleProductOffering.getLifecycleStatus().equals(ProductOfferingLifecycle.RETIRED) && !bundleProductOffering.getLifecycleStatus().equals(ProductOfferingLifecycle.OBSOLETE)) {
				return false;
			}
		 }
		}
