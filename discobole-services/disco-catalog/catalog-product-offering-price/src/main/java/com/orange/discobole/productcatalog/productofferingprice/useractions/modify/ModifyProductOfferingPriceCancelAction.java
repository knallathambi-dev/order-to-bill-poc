 ...
 *
 */
@Component("ProductOfferingPriceModification.productOfferingPriceModificationCancel")
public class ModifyProductOfferingPriceCancelAction implements UserAction {

	private static final Logger LOGGER = LogManager.getLogger(ModifyProductOfferingPriceCancelAction.class);
	...
	public Map<String, Object> perform(StateMachineTransition stateMachine, TaskFlowUpdate taskFlowUpdate)
		...
		LOGGER.info("In perform method of ModifyProductOfferingPriceCancelAction");
		...
		if (null != characteristic) {
			...
			if (cancelData.isIsCancelled().equals(Boolean.TRUE)) {
				...
				if (productOfferingPriceId != null) {
					productOfferingPriceService.cancelProductOfferingPriceModification(productOfferingPriceId);
				}
			} 
		}
