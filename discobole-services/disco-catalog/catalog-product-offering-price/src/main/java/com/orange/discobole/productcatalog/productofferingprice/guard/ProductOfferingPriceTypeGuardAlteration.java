 ...
 * @since 1.0
 */
@Component("isPriceTypePOPA")
public class ProductOfferingPriceTypeGuardAlteration implements StateMachineGuard<String,String> {
	
	/**
	 * this guard check if selected product offering price is of type pop alteration, if
	...
	public Mono<Boolean> apply(StateContext<String, String> context) {
		return Mono.just(context.getExtendedState().getVariables().get(ProductOfferingPriceConstants.SELECT_POP_TYPE)
				.equals(ProductOfferingPriceType.PRODUCTOFFERINGPRICEALTERATION.toString()));
	}
}
