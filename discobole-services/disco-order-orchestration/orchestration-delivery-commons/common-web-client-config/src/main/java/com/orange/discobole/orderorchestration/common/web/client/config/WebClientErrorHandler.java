...
import com.orange.discobole.orderorchestration.exception.AuthenticationException;
...
public class WebClientErrorHandler implements ExchangeFilterFunction {
    ...
    private Mono<ClientResponse> handleError(ClientResponse response) {
        ...
        if (statusCode.isSameCodeAs(HttpStatus.UNAUTHORIZED)) {
            return Mono.error(new AuthenticationException("Missing credentials"));
        }
        HttpStatus httpStatus = HttpStatus.resolve(statusCode.value());
        if (httpStatus == null) {
            response.mutate().statusCode(HttpStatus.INTERNAL_SERVER_ERROR);
        }

        return response.bodyToMono(String.class).flatMap(s -> Mono.error(new DiscoException(s)));
    }
}
