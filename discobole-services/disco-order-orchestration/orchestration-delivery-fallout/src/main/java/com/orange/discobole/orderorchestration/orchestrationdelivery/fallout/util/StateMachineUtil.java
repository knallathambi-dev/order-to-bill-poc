...
public class StateMachineUtil {
    ...
    public static <T> T getObjectValue(StateContext<String, String> context, String keyName, TypeReference<T> toValueTypeRef) {
        try {
            Object o = context.getExtendedState().getVariables().get(keyName);
            if (o != null) {
        ...
        if (object instanceof List<?> list) {
            ...
            for (Object item : list) {
                if (elementType.isInstance(item)) {
                    ...
                    throw new IllegalArgumentException("Element in the list is not of the expected type");
                }
            }
            return typedList;
        } else {
            throw new IllegalArgumentException("Key does not correspond to a List in the map");
        }
    }
