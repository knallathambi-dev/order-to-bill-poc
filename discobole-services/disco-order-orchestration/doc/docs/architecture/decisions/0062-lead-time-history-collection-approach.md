...
| Option | Integrating the calculation with node state updates (../
|--------|-----------------------------------------------------------|------------------------------------------------------------------------|------------------|
| **Description** | Update the lead time history statistics within the code of the different functions that create lead time (../
| **Statistics Coverage** | - Doesn't lose any history statistic storage.<br>- History statistics stored match exactly the different plans/nodes processed. | - History statistics stored are delayed from the actual processed plans/nodes.<br>- Might lose some statistics. | - History lead times are delayed one hour from the actual updates.<br>- Statistics for the last hour are not generated (../
...
- Implementing business logic using database pipelines or hooks (../
...
1. The cron job has the lowest delay among the proposed solutions, and it also needs to define the impact on error handling (../
2. For the following solution (../
   Integrating the calculation with node state updates (../
