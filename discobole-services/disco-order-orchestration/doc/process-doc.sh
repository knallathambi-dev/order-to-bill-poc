...
# Iterates over all projects from given group with given visibility and tries to process it as a disco-order-orchestration module project
# $1: GitLab group path
...
function process_modules_from_group() {
  ...
  visibility="$2"
  exclude_projects="$3"
  group_name=$(echo "$group_path" | sed -e 's/disco-order-orchestration\///g')
...
function build_architecture_decisions_toc() {
  ...
  git clone -q --depth 1 "https://gitlab-ci-token:${CI_JOB_TOKEN}@${CI_SERVER_HOST}/disco/disco-oda-components/disco-order-orchestration/orchestration-delivery-architecture.git" "$adr_src_dir"
...
log_info "=========================================================================================="
log_info "\\e[33;1mprocess-doc.sh\\e[0m processes COOD module:"
...
# process all disco-order-orchestration module toc
...
then
  ...
  toc=$(< "$tmp_dir/api_user_guides_toc.md" sed 's/^/  /' | awk '{gsub(/&/, "\\\\&"); printf "%s\\n", $0}')
  awk -v toc="${toc}" '/INSERT_API_USER_GUIDES_TOC_HERE/ {print toc; next} 1' "${tmp_dir}/mkdocs-with-toc-tmp2.yml" > "${tmp_dir}/mkdocs-with-toc-tmp3.yml"

  log_info "Replacing API compliance report toc in mkdocs.yml..."
  toc=$(< "$tmp_dir/api_compliance_reports_toc.md" sed 's/^/  /' | awk '{gsub(/&/, "\\\\&"); printf "%s\\n", $0}')
  awk -v toc="${toc}" '/INSERT_API_COMPLIANCE_REPORTS_TOC_HERE/ {print toc; next} 1' "${tmp_dir}/mkdocs-with-toc-tmp3.yml" > "${tmp_dir}/mkdocs-with-toc-tmp4.yml"

  log_info "Replacing API conformance report toc in mkdocs.yml..."
  toc=$(< "$tmp_dir/api_conformance_reports_toc.md" sed 's/^/  /' | awk '{gsub(/&/, "\\\\&"); printf "%s\\n", $0}')
  awk -v toc="${toc}" '/INSERT_API_CONFORMANCE_REPORTS_TOC_HERE/ {print toc; next} 1' "${tmp_dir}/mkdocs-with-toc-tmp4.yml" > "${tmp_dir}/mkdocs-with-toc-tmp5.yml"
