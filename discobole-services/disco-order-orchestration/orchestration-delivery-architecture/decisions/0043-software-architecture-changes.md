...
The objective of this ADR is to enhance the Kafka topic names used in the Order Orchestration Service (COOD) and align the service with TeleManagement Forum (TMF) concepts related to publishing state change events. Additionally, the ADR aims to improve plan flow execution and delivery by leveraging state change events.
...
- **SOM State Change Topic**: Introduced to indicate the status of Container Freight Station (CFS) delivery. The name will be adjusted after the introduction of the delivery management service.
