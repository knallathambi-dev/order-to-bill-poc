...
The objective of this Architecture Decision Record (ADR) is to evaluate the most efficient approach for the COOD database, design patterns, and microservices.

## Considered Options

We considered two design patterns: CQRS (Command Query Responsibility Segregation) and CRUD (Create, Read, Update, Delete).
...
[Link to Proposed Solution of COOD Component](link here)
...
3. Listeners will handle different topics: OM Topic (triggering the Order Delivery process), Orchestration Topic, Delivery Topic, and Delivery Status Topic.
