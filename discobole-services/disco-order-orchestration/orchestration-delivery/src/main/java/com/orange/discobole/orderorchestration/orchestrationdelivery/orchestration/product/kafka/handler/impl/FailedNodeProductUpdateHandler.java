...
@Component
public class FailedNodeProductUpdateHandler extends ProductUpdateHandler {

    public FailedNodeProductUpdateHandler(ProductManagementService productManagementService) {
        super(productManagementService);
    }

    @Override
    public boolean isEligibleForUpdate(OrchestrationPlanNode node) {
        return OrchestrationPlanNodeState.FAILED.equals(node.getState());
    }
    ...
    protected void updateCfsProduct(Product product, OrchestrationPlanNode node) {
        ...
        switch (ProductOrderItemActionType.fromValue(orderItem.getAction())) {
            ...
            case DELETE, MODIFY ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.ACTIVE);
            default ->
