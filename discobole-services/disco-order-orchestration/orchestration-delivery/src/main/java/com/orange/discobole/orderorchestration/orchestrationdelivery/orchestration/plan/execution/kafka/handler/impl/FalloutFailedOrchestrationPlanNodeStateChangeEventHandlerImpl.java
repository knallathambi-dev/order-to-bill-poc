...
@Service
public class FalloutFailedOrchestrationPlanNodeStateChangeEventHandlerImpl extends FalloutCommonOrchestrationPlanNodeStateChangeEventHandler {

    public FalloutFailedOrchestrationPlanNodeStateChangeEventHandlerImpl(ProcessFlowManagement processFlowManagement) {
        super(processFlowManagement);
    }

    @Override
    public void handleEvent(OrchestrationPlanNode node, Map<String, String> headers) {
        if (!OrchestrationPlanNodeState.FAILED.equals(node.getState())) {
        ...
        if (isPreviousStateHeld) {
            ...
            processFlowManagement.submitResolutionSateWithReason(nextTaskUrl, ResolutionState.UNRESOLVED, "automatic resolution");
        }
    }
}
