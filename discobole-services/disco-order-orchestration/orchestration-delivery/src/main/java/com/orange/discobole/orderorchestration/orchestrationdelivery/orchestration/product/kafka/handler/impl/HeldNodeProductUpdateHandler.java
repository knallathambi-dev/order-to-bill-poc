...
@Component
public class HeldNodeProductUpdateHandler extends ProductUpdateHandler {

    public HeldNodeProductUpdateHandler(ProductManagementService productManagementService) {
        super(productManagementService);
    }

    @Override
    public boolean isEligibleForUpdate(OrchestrationPlanNode node) {
        return OrchestrationPlanNodeState.HELD.equals(node.getState());
    }
    ...
    protected void updateCfsProduct(Product product, OrchestrationPlanNode node) {
        ...
        switch (ProductOrderItemActionType.fromValue(orderItem.getAction())) {
            case ADD, MIGRATE ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.LOCKED);
            case MODIFY, DELETE ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.LOCKEDACTIVE);
            default ->
    ...
    protected void updateTangibleProduct(Product product, OrchestrationPlanNode node) {
        ...
        if (ProductOrderItemActionType.ADD.getValue().equals(orderItem.getAction())) {
            productManagementService.updateTangibleOperationalStatus(product, ProductOperationalStatusType.LOCKED);
        }
    }
}
