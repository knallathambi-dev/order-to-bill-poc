...
@Component
public class InDeliveryNodeProductUpdateHandler extends ProductUpdateHandler {

    public InDeliveryNodeProductUpdateHandler(ProductManagementService productManagementService) {
        super(productManagementService);
    }

    @Override
    public boolean isEligibleForUpdate(OrchestrationPlanNode orchestrationPlanNode) {
        return OrchestrationPlanNodeState.IN_DELIVERY.equals(orchestrationPlanNode.getState());
    }
    ...
    protected void updateCfsProduct(Product product, OrchestrationPlanNode node) {
        ...
        switch (ProductOrderItemActionType.fromValue(orderItem.getAction())) {
            case ADD, MIGRATE ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.PENDINGACTIVE);
            case MODIFY ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.PENDINGMODIFICATION);
            case DELETE ->
                    productManagementService.updateCFSOperationalStatus(product, ProductOperationalStatusType.PENDINGTERMINATE);
            default ->
    ...
    protected void updateTangibleProduct(Product product, OrchestrationPlanNode node) {
        ...
        productManagementService.updateTangibleOperationalStatus(product, ProductOperationalStatusType.PENDINGDELIVERY);
    }
}
