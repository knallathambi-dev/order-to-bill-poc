...
function process_module() {
  ...
  log_info " ... downloading \\e[33;1mREADME.md\\e[0m to \\e[33;1m$DOC_OUT/$MODULE_DIR/$project_name.md\\e[0m..."
...
# Iterates over all projects from given group with given visibility and tries to process it as a lizard module project
# $1: GitLab group path
...
function process_modules_from_group() {
  ...
  do
    ...
    then
      ...
      group_name=$(echo "$project_json" | jq -r .namespace.name)
...
function build_architecture_decisions_toc() {
  ...
  git clone -q --depth 1 "https://gitlab-ci-token:${CI_JOB_TOKEN}@${CI_SERVER_HOST}/disco/disco-oda-components/disco-security/security-authorization-architecture.git" "$adr_src_dir"

  # retrieve ADR files from source
  ...
  for adr_file in "$adr_dst_dir"/*.md; do
    log_info " ... processing $adr_file ..."
    [[ -f "$adr_file" ]] && sed -i -e "s/([^0-9]*/(..\//g" -e "s/\.md)/)/g" "$adr_file"
  done
  ...
  do
    if [ -f "$adr_file" ]; then
      log_info " ... adding $adr_file to architecture decisions TOC ..."
      ...
      log_info " ... no ADR files found in $adr_dst_dir"
      continue
    fi
  done
...
log_info "=========================================================================================="
log_info "\\e[33;1mprocess-doc.sh\\e[0m processes User Role & Permission Mgt module:"
