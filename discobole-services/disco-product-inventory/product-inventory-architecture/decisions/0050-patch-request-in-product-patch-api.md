...
# PATCH request in Product patch API (JSON merge PATCH or JSON PATCH)
...
* RFC6902 (JSON Patch)
* JSON Patch Query (extention of RFC6902)

## Decision Outcome

To use 3rd option with the jsonpath as path and make patch request by our code not by JsonPatch liberary (as it didn't support the JsonPath)
...
DocumentContext context = JsonPath.parse(writeValueAsString);
List<List<String>> operations = List.of(List.of("productRelationship[?(@.relationshipType == 'bundles')].product.status",
        "replace", "ABORTED"));
for (List<String> operation : operations) {
    ...
    switch (op) {
        case "add":
            context = context.add(path, value);
            break;
        case "replace":
        ...
        case "test":
            ...
            if (!context.read(path).equals(value)) {
...
ProductEntity updatedProduct = objectMapper.convertValue(updatedDocument, ProductEntity.class);
productMapper.toSameEntity(updatedProduct, product);
mongoTemplate.save(product);
```

## Pros and Cons of the Options

| Aspect | RFC7386 (JSON Merge Patch) | RFC6902 (JSON Patch) | JSON Patch Query (extention of RFC6902) | 
| ------ | -------------------------- | -------------------- | --------------------------------------- | 
...
| Test Operation | Not applicable (no test operation) | Supported | Supported |
...
> JSON Patch Query (extention of RFC6902): other format of path it can use JSON path as mentioned in TMF-630 example
...
* TMF 630 part 6 support jsonpath for patch operations, it will be helpful to use it directly as standard in json parsing `"path": "note[?(@.author=='John Doe')] "`.
...
Example JSONPath: $.property[?(@.innerProperty == 'value')]
...
* Supports a richer syntax with operators like @, *, ?(), [], and more.
