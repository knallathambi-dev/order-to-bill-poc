...
public enum ErrorCodeEnum {
    ...
    METHOD_NOT_ALLOWED("Method not allowed", 61), CONFLICT("Conflict", 69), NOT_IMPLEMENTED("Not Implemented", 501);
    ...
    @JsonCreator
    public static ErrorCodeEnum fromStatus(String status) throws ProductInventoryException {
        for (ErrorCodeEnum errorCodeEnum : ErrorCodeEnum.values()) {
            if (errorCodeEnum.status.equalsIgnoreCase(status)) {
                return errorCodeEnum;
            }
        }
        throw new ProductInventoryException("Invalid ErrorCodeEnum status: " + status);
    }


    @JsonCreator
    public static ErrorCodeEnum fromCode(int code) throws ProductInventoryException {
        for (ErrorCodeEnum errorCodeEnum : ErrorCodeEnum.values()) {
            if (errorCodeEnum.code == code) {
                return errorCodeEnum;
            }
        }
        throw new ProductInventoryException("Invalid ErrorCodeEnum code: " + code);
    }
