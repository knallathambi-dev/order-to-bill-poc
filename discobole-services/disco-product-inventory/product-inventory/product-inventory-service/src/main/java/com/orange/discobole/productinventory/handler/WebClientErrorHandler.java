...
public class WebClientErrorHandler implements ExchangeFilterFunction {
    ...
    private Mono<ClientResponse> handleError(ClientRequest request, Throwable throwable) {
        ...
        return Mono.error(new ProductInventoryException(String.format("Error while call %s with method %s", request.url(), request.method()), throwable));
    }
    ...
    private Mono<ClientResponse> handleError(ClientResponse response) {
        ...
        if (statusCode.isSameCodeAs(HttpStatus.UNAUTHORIZED)) {
            return Mono.error(new ProductInventoryException(HttpStatus.UNAUTHORIZED, MISSING_CREDENTIALS.getCode(), MISSING_CREDENTIALS.getStatus()));
        }
        HttpStatus httpStatus = HttpStatus.resolve(statusCode.value());
        if (httpStatus == null) {
            httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
        }
        return Mono.error(new ProductInventoryException(httpStatus, statusCode.value(), "An error occurred during the request."));
    }
}
