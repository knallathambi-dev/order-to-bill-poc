...
import com.orange.discobole.productinventory.dto.kafka.ProductCreateEventPayload;
...
@Slf4j
public class ProductCreateEventProducerImpl implements EventProducer<Product> {

    private final KafkaTemplate<String, ProductCreateEvent> kafkaTemplate;


    private ProductCreateEvent createProductCreateEvent(Product product, String title, String domain) {
        ProductCreateEventPayload productCreateEventPayload =
        ...
        return ProductCreateEvent
                ...
                .title(title)
                .domain(domain)
                .eventType(EventType.PRODUCT_CREATE_EVENT.getValue())
                .event(productCreateEventPayload)
                .build();
    }
    ...
    public void publishEvent(Product product, String title, String domain) {
        ...
        ProductCreateEvent productCreateEvent = createProductCreateEvent(product, title, domain);
        String partitionKey = product.getId();
        Message<ProductCreateEvent> message = MessageBuilder.withPayload(productCreateEvent).build();
        kafkaTemplate.send(Constant.PRODUCT_CREATE_EVENT_TOPIC, partitionKey, message.getPayload())
                .thenAccept(sendResult ->
                        log.info("{}  is published on topic {} for partition {}",
                                "ProductCreateEvent",
                                partitionKey,
                                Constant.PRODUCT_CREATE_EVENT_TOPIC)
                );
