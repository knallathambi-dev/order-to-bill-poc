...
DISCOBOLE Product Inventory Micro Frontend is a standalone microfrontend for **Product Management** within the DISCOBOLE Admin
platform. Built with **Vite**, **React**, and **Module Federation**, it implements Product Inventory management
capabilities and can run independently or be consumed by the host shell at runtime.

The `Product Inventory UI` MFE provides:

- **product viewing**: displays product details, hierarchies, and statuses with comprehensive filtering and sorting
- **job monitoring**: tracks job specifications, executions, and reports
- **reporting**: generates charts and reports on product and job data
...
```bash
git clone git@gitlab.ow2.org:disco/disco-oda-components/disco-ui-portals/disco-admin-ui/product-inventory-ui.git
```
...
curl -s https://gitlab.ow2.org/api/v4/projects/discobole%2Fdisco-oda-components%2Fdisco-ui-portals%2F/disco-admin-ui%2Fproduct-inventory-ui | jq .id
...
| Mode    | Port   | URL                                         |
|---------|--------|---------------------------------------------|
| Dev     | `3005` | http://localhost:3005                       |
| Preview | `3002` | http://localhost:3002                       |
...
**Note:** ensure that the backend services (Product Inventory API) are accessible and properly configured.

## What It Does

- **Displays and manages products**: list view with filtering, sorting, and detailed product information
- **Provides product details**: comprehensive view of product hierarchies and statuses
- **Monitors jobs**: tracks job specifications, executions, and generates reports
...
docker run -p 3003:8080 product-inventory-ui
```
...
- [Component documentation](https://discobole.ow2.io/disco-oda-components/disco-ui-portals/disco-microfrontend/product-inventory-ui/doc)
